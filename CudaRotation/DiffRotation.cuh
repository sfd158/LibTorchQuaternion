...
namespace DiffRotationKernel
{
    ...
    template<typename data_type>
    __global__ void quat_integrate_backward(const data_type * q, const data_type * omega, data_type dt, const data_type * grad_in, data_type * q_grad, data_type * omega_grad, size_t num_quat)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num_quat; i += gridDim.x * blockDim.x)
        {
    ...
    __global__ void mat22_det(const data_type* x, data_type * res, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::mat22_det<data_type>(x + 4 * i, res[i]);
        }
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    __global__ void mat22_det_backward(const data_type* x, const data_type* grad_in, data_type* grad_out, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::mat22_det_backward<data_type>(x + 4 * i, grad_in[i], grad_out + 4 * i);
        }
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    __global__ void mat33_det(const data_type* x, data_type * result, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::mat33_det<data_type>(x + 9 * i, result[i]);
        }
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    __global__ void mat33_det_backward(const data_type* x, const data_type * grad_in, data_type * grad, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::mat33_det_backward<data_type>(x + 9 * i, grad_in[i], grad + 9 * i);
        }
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    __global__ void mat33_inverse(const data_type* x, data_type * res, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::mat33_inverse<data_type>(x + 9 * i, res + 9 * i);
        }
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    __global__ void quat_from_matrix(const data_type * mat, data_type * q, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::quat_from_matrix<data_type>(mat + 9 * i, q + 4 * i);
        }
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    __global__ void quat_from_matrix_backward(const data_type * mat, const data_type * grad_in, data_type * grad_out, size_t num)
    {
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num; i += gridDim.x * blockDim.x)
        {
            DiffRotationSingleKernel::quat_from_matrix_backward<data_type>(mat + 9 * i, grad_in + 4 * i, grad_out + 9 * i);
        }
    }
...
namespace DiffRotationImplVoid
{
    ...
    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void mat22_det(const void * x, void * res, size_t num, int grid, int block)
    {
        DiffRotationKernel::mat22_det<data_type> <<<grid, block>>>(static_cast<const data_type*>(x), static_cast<const data_type*>(res), num);
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void mat22_det_backward(const void* x, const void* grad_in, void* grad_out, size_t num, int grid, int block)
    {
        DiffRotationKernel::mat22_det_backward<data_type> <<<grid, block>>> (static_cast<const data_type *>(x), static_cast<const data_type *>(grad_in), static_cast<data_type *> (grad_out), num);
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void mat33_det(const void* x, void* result, size_t num, int grid, int block)
    {
        DiffRotationKernel::mat33_det<data_type> <<<grid, block >>> (static_cast<const data_type*>(x), static_cast<data_type*>(result), num);
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void mat33_det_backward(const void* x, const void* grad_in, void* grad, size_t num, int grid, int block)
    {
        DiffRotationKernel::mat33_det_backward<data_type> <<<grid, block >>> (static_cast<const data_type *>(x), static_cast<const data_type *>(grad_in), static_cast<data_type *>(grad), num);
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void mat33_inverse(const void* x, void* res, size_t num, int grid, int block)
    {
        DiffRotationKernel::mat33_inverse<data_type> <<<grid, block >>> (static_cast<const data_type *>(x), static_cast<data_type *>(res), num);
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void quat_from_matrix(const void* mat, void* q, size_t num, int grid, int block)
    {
        DiffRotationKernel::quat_from_matrix<data_type> <<<grid, block >>> (static_cast<const data_type*>(mat), static_cast<data_type*>(q), num);
    }

    template<class data_type, class = typename std::enable_if<std::is_floating_point<data_type>::value>::type>
    void quat_from_matrix_backward(const void* mat, const void* grad_in, void* grad_out, size_t num, int grid, int block)
    {
        DiffRotationKernel::quat_from_matrix_backward<data_type> <<<grid, block >>>(static_cast<const data_type*>(mat), static_cast<const data_type*>(grad_in), static_cast<data_type*>(grad_out), num);
    }
}