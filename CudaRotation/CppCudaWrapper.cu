...
namespace DiffRotationImpl
{
    ...
    torch::Tensor QuatFromMatrix::forward(AutogradContext * ctx, torch::Tensor x)
    {
        ...
        *shape.rbegin() = 4;
        ...
        if (device.is_cuda())
        {
            int grid, block;
            CudaConfig::get_thread_block(batch_size, grid, block, device.index());
            if (dtype == torch::kFloat32) DiffRotationImplVoid::quat_from_matrix<float>(x.data_ptr(), result.data_ptr(), batch_size, grid, block);
            else if (dtype == torch::kFloat64) DiffRotationImplVoid::quat_from_matrix<double>(x.data_ptr(), result.data_ptr(), batch_size, grid, block);
            else throw std::runtime_error("dtype not supported.");
        }
        else if (device.is_cpu())
        {
    ...
    variable_list QuatFromMatrix::backward(AutogradContext * ctx, variable_list grad_output)
    {
        ...
        if (device.is_cuda())
        {
            int grid, block;
            CudaConfig::get_thread_block(batch_size, grid, block, device.index());
            if (dtype == torch::kFloat32) DiffRotationImplVoid::quat_from_matrix_backward<float>(mat.data_ptr(), grad_o.data_ptr(), result.data_ptr(), batch_size, grid, block);
            else if (dtype == torch::kFloat64) DiffRotationImplVoid::quat_from_matrix_backward<double>(mat.data_ptr(), grad_o.data_ptr(), result.data_ptr(), batch_size, grid, block);
            else throw std::runtime_error("dtype not supported.");
        }
        else if (device.is_cpu())
        {
